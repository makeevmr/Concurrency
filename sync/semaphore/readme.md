...
Семафор – это автомат с жетонами (_tokens_, _permits_), который поддерживает две операции:
...
1) Реализуйте [считающий семафор](source/semaphore.hpp) с помощью условных переменных.

2) С помощью семафоров реализуйте [блокирующую очередь](source/queue.hpp) для передачи данных между потоками.
...
Обе эти проблемы решает [`TaggedSemaphore`](source/tagged_semaphore.hpp):

- Теперь жетон имеет явное представление – `Permit`. Получить жетон можно только из вызова `Acquire`. Копировать жетон нельзя, можно только перемещать.
