...
(Этот класс можно реализовать через `std::atomic<size_t>` c методами `fetch_add` + `load`, но можно сделать и еще лучше...)
...
Программа на С++, содержащая data race, является некорректной: это UB, поведение такой программы не определено семантикой языка.
